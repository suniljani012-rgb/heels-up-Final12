    ...
    if (path === '/register' && method === 'POST') {
        try {
            ...
            const result = await env.DB.prepare(
                "INSERT INTO users (first_name, last_name, email, phone, password_hash, role, email_verified, created_at, updated_at) VALUES (?, ?, ?, ?, ?, 'customer', ?, ?, ?)"
            ).bind(firstName, lastName, email, phone, hashed, requireOtp !== 'false' ? 1 : 0, now, now).run();
