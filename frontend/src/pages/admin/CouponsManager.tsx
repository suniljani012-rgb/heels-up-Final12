  ...
  return (
    <div className="space-y-6 text-white animate-fade-in relative">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-light text-white font-display italic">Promo Codes Registry</h1>
          <p className="text-xs text-neutral-400">Manage campaign discounts and customer checkout coupons</p>
        </div>
        <button
            ...
            <tbody className="divide-y divide-neutral-900/60">
              {filtered.map((c) => (
                <tr key={c.id} className="hover:bg-[#121211]/20 transition-colors">
                  ...
                  <td className="p-4 text-neutral-400 capitalize">{c.type}</td>
                  <td className="p-4 font-mono font-bold text-neutral-200">
                    {c.type === 'percentage' ? `${c.value}%` : `₹${c.value}`}
                  </td>
                  <td className="p-4 font-mono text-neutral-400">₹{(c.min_order / 100).toFixed(0)}</td>
                  <td className="p-4 font-mono text-neutral-400">{c.used_count} / {c.max_uses || '∞'}</td>
                  <td className="p-4 font-mono text-neutral-400">{c.expires_at ? c.expires_at.split(' ')[0] : 'Never'}</td>
                  <td className="p-4">
                    <span className={`px-2 py-0.5 rounded-full text-[9px] font-bold uppercase tracking-wider ${c.active ? 'bg-emerald-500/10 border border-emerald-500/20 text-emerald-400' : 'bg-neutral-800 text-neutral-500'}`}>
              ...
              {filtered.length === 0 && (
                <tr>
                  <td colSpan={8} className="py-24 text-center text-neutral-500 italic font-mono bg-[#0f0f0e]">No promo codes match criteria.</td>
                </tr>
              )}
            </tbody>
          </table>
          ...
          <div className="w-full max-w-md bg-[#0e0e0d] border-l border-neutral-900 shadow-2xl relative z-10 p-6 flex flex-col justify-between h-full overflow-y-auto">
            <div className="space-y-6">
              ...
              <form onSubmit={handleSubmit} className="space-y-5 text-xs">
                <div>
                  <label className="block text-[10px] uppercase tracking-wider font-bold text-neutral-400 mb-1">Coupon Code</label>
                  <input
                    type="text"
                    required
                    value={code}
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-[10px] uppercase tracking-wider font-bold text-neutral-400 mb-1">Discount Type</label>
                    <select
                      value={type}
                  ...
                  <div>
                    <label className="block text-[10px] uppercase tracking-wider font-bold text-neutral-400 mb-1">Discount Value</label>
                    <input
                      type="number"
                      required
                      value={value}
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-[10px] uppercase tracking-wider font-bold text-neutral-400 mb-1">Min Spend Required (₹)</label>
                    <input
                      type="number"
                      value={minOrder}
                  ...
                  <div>
                    <label className="block text-[10px] uppercase tracking-wider font-bold text-neutral-400 mb-1">Max Cap (Percentage only, ₹)</label>
                    <input
                      type="number"
                      value={maxDiscount}
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-[10px] uppercase tracking-wider font-bold text-neutral-400 mb-1">Max Uses Limit</label>
                    <input
                      type="number"
                      value={maxUses}
                  ...
                  <div>
                    <label className="block text-[10px] uppercase tracking-wider font-bold text-neutral-400 mb-1">Expiration Date</label>
                    <input
                      type="date"
                      value={expiresAt}
                ...
                <div>
                  <label className="block text-[10px] uppercase tracking-wider font-bold text-neutral-400 mb-1">Campaign Description</label>
                  <textarea
                    rows={2}
                    value={description}
                    onChange={(e) => setDescription(e.target.value)}
                    placeholder="e.g. 15% off Oxford Jodhpurs on order above 3999"
                    className="w-full bg-[#121211] border border-neutral-850 rounded-xl px-3 py-2 text-white focus:outline-none leading-relaxed"
                  />
                </div>
                ...
                <div>
                  <label className="relative inline-flex items-center cursor-pointer">
                    ...
                    <span className="ml-2 text-neutral-450 text-[10px] font-bold">Coupon Code Active</span>
                  </label>
                </div>
