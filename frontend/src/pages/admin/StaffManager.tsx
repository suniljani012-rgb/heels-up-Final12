  ...
  return (
    <div className="space-y-6 text-white animate-fade-in relative">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-light text-white font-display italic">Staff Management</h1>
          <p className="text-xs text-neutral-400">Manage internal users, role access models, and session scopes</p>
        </div>
        <button
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse text-xs">
            ...
            <tbody className="divide-y divide-neutral-900/60">
              {filtered.map((s) => (
                <tr key={s.id} className="hover:bg-[#121211]/20 transition-colors">
                  <td className="p-4 font-bold text-white text-xs">{s.name || 'Anonymous User'}</td>
                  <td className="p-4 font-mono text-neutral-350">{s.email}</td>
                  <td className="p-4 font-mono font-bold text-primary capitalize">{s.role}</td>
                  <td className="p-4">
                    <span className={`px-2 py-0.5 rounded text-[9px] font-bold uppercase ${s.two_factor_enabled ? 'bg-emerald-500/10 border border-emerald-500/20 text-emerald-400' : 'bg-neutral-850 text-neutral-500'}`}>
                      {s.two_factor_enabled ? 'Verified' : 'Inactive'}
                    </span>
                  </td>
                  <td className="p-4">
                    <span className={`px-2 py-0.5 rounded-full text-[9px] font-bold uppercase tracking-wider ${s.active ? 'bg-emerald-500/10 border border-emerald-500/20 text-emerald-400' : 'bg-neutral-800 text-neutral-500'}`}>
              ...
              {filtered.length === 0 && (
                <tr>
                  <td colSpan={6} className="py-24 text-center text-neutral-500 italic font-mono bg-[#0f0f0e]">No staff credentials match criteria.</td>
                </tr>
              )}
            </tbody>
          </table>
          ...
          <div className="w-full max-w-md bg-[#0e0e0d] border-l border-neutral-900 shadow-2xl relative z-10 p-6 flex flex-col justify-between h-full overflow-y-auto">
            <div className="space-y-6">
              <div className="flex items-center justify-between border-b border-neutral-900 pb-4">
                <h3 className="text-sm font-bold uppercase tracking-widest text-primary font-mono flex items-center gap-1.5">
                  <ShieldCheck className="w-5 h-5 text-primary" /> {editingStaff ? 'Modify Staff Credentials' : 'Register Staff Account'}
                </h3>
              ...
              <form onSubmit={handleSubmit} className="space-y-5 text-xs">
                <div>
                  <label className="block text-[10px] uppercase tracking-wider font-bold text-neutral-400 mb-1">Email Address</label>
                  <input
                    type="email"
                    required
                    value={email}
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-[10px] uppercase tracking-wider font-bold text-neutral-400 mb-1">Role Type</label>
                    <select
                      value={role}
                  ...
                  <div>
                    <label className="block text-[10px] uppercase tracking-wider font-bold text-neutral-400 mb-1">Access Status</label>
                    <div className="flex items-center gap-2 pt-2">
                      <label className="relative inline-flex items-center cursor-pointer">
                        ...
                        <span className="ml-2 text-neutral-450 text-[10px] font-bold">Access Granted</span>
                      </label>
                    </div>
                  </div>
                </div>
