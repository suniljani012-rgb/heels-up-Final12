      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-[#0f0f0e] border border-neutral-900 p-4 rounded-xl">
          ...
          <div className="flex items-center gap-1.5 mt-1">
            <span className="text-base font-bold font-mono text-white">{stats.avg}</span>
            <div className="flex text-amber-500">
              <Star className="w-3.5 h-3.5 fill-current" />
            </div>
          </div>
          <span className="text-[7px] text-neutral-400 font-semibold mt-1 block">Out of 5 Stars</span>
        </div>
        <div className="bg-[#0f0f0e] border border-neutral-900 p-4 rounded-xl">
          <span className="block text-[8px] font-bold text-neutral-500 uppercase tracking-wider">Total reviews</span>
          <span className="block text-base font-bold font-mono text-white mt-1">{stats.total} entries</span>
          <span className="text-[7px] text-[#ead2ae] font-semibold mt-1 block">Store overall feedback</span>
        </div>
        <div className="bg-[#0f0f0e] border border-neutral-900 p-4 rounded-xl">
        ...
        <div className="bg-[#0f0f0e] border border-neutral-900 p-4 rounded-xl">
          ...
          <span className="block text-base font-bold font-mono text-emerald-400 mt-1">{stats.approved} items</span>
        ...
        {filteredReviews.length === 0 ? (
          ...
          <div className="divide-y divide-neutral-900/60">
            {filteredReviews.map(rev => (
              <div key={rev.id} className="p-5 flex flex-col md:flex-row gap-6 hover:bg-[#121211]/20 transition-colors">
                ...
                <div className="md:w-56 shrink-0 space-y-1.5">
                  ...
                  <div>
                    <h4 className="text-xs font-bold text-white leading-tight">{rev.reviewer_name}</h4>
                  ...
                  <div>
                    ...
                    <span className="text-[10px] text-neutral-400 font-semibold line-clamp-1">{rev.product_name}</span>
                  </div>
                  <div>
                    {rev.approved ? (
                ...
                <div className="flex-1 space-y-3">
                  <div>
                    <h3 className="text-[11px] font-bold text-white uppercase tracking-wider">{rev.title}</h3>
                    <p className="text-xs text-neutral-400 leading-relaxed mt-1 italic font-light">"{rev.body}"</p>
                  </div>
                  
                  {rev.merchant_reply && (
                    <div className="p-3 bg-[#121211]/80 border border-neutral-900/50 rounded-xl space-y-1">
                      <span className="block text-[8px] font-bold text-[#ead2ae] uppercase tracking-wider">Merchant response:</span>
                      <p className="text-[10px] text-neutral-400 font-medium leading-relaxed">"{rev.merchant_reply}"</p>
                    </div>
                  )}
                  ...
                  <div className="flex gap-4 pt-1">
                    ...
                    <button
                      ...
                      className="px-2.5 py-1.5 border border-neutral-800 text-white font-semibold rounded-lg text-[9px] uppercase tracking-wider flex items-center gap-1 hover:bg-neutral-850 transition-colors"
                    >
          ...
          <div className="w-full max-w-lg bg-[#0e0e0d] border-l border-neutral-900 shadow-2xl relative z-10 p-6 flex flex-col justify-between h-full overflow-y-auto">
            <div className="space-y-6">
              <div className="flex items-center justify-between border-b border-neutral-900 pb-4">
                <h3 className="text-sm font-bold uppercase tracking-wider text-white">Merchant Response Console</h3>
                <button
              ...
              <div className="p-4 bg-[#121211] border border-neutral-850 rounded-xl space-y-2">
                ...
                <h4 className="text-[10px] font-bold text-white uppercase">{selectedReview.title}</h4>
                <p className="text-[11px] text-neutral-400 leading-relaxed italic">"{selectedReview.body}"</p>
              ...
              <form onSubmit={handleReplySubmit} className="space-y-4">
                <div>
                  <label className="block text-[9px] font-bold text-neutral-400 uppercase tracking-wider mb-1.5">Official Merchant Reply Message</label>
                  <textarea
                    required
                    rows={6}
                    value={replyText}
                    ...
                    className="w-full bg-[#121211] border border-neutral-850 rounded-xl p-4 text-xs text-white focus:outline-none focus:border-amber-500/60"
                  />
                </div>
                <button
                  type="submit"
