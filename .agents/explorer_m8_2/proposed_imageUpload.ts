...
// - HEIC/HEIF is converted natively on Safari using HTML5 Canvas.
// - On Chrome/Firefox (which don't natively decode HEIC):
//   - In production: Uploaded RAW to the server, where the server converts it using Cloudflare Image Resizing.
//   - In local dev: Converted on the client using a dynamically imported 'heic2any' library to avoid server-side resizing errors.
// - This completely avoids client-side hangs from heic2any in production for Windows/Chrome users.
...
// Helper to check if running in local development environment
...
export async function prepareImageFile(file: File): Promise<File> {
  ...
  const isHeicFile = ext === 'heic' || ext === 'heif' || file.type === 'image/heic' || file.type === 'image/heif';

  if (isHeicFile) {
    // 1. Try native decoding first (Safari supports HEIC natively)
    try {
      const img = await blobToImg(file);
      return await canvasToPNG(img, file.name);
    } catch (err) {
      // Native decoding failed (e.g. Chrome/Firefox on Windows/Linux)
      // 2. If in local dev, fall back to client-side conversion using dynamically imported heic2any
      if (isLocalHostname()) {
        try {
          console.log('Local dev: Converting HEIC file to PNG client-side...');
          // @ts-ignore
          ...
          // Now feed the converted PNG through the canvas resize pipeline
          const img = await blobToImg(pngFile);
          return await canvasToPNG(img, pngFile.name);
        } catch (convErr) {
          console.error('Client-side HEIC conversion failed:', convErr);
          return file; // upload raw HEIC as fallback
        }
      }
      
      // 3. In production, upload raw HEIC as-is (server will convert via CF Image Resizing)
      return file;
    }
  }
